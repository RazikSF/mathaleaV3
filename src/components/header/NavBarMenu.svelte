...
<div class="group inline-block relative" use:clickOutside on:click_outside={handleClickOutside}>
  <button class="h-16 bg-coopmaths hover:bg-coopmaths-light  text-white text-xl font-extrabold relative flex lg:block py-2 px-2 lg:px-8 items-center" {id} on:click={() => (isMenuOpen = !isMenuOpen)} on:keydown={() => (isMenuOpen = !isMenuOpen)}>
